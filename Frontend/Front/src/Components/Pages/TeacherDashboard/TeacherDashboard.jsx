    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .td-code-panel { display: flex; flex-direction: column; align-items: flex-end; gap: 0.4rem; }
    .td-code-label { font-size: 0.72rem; color: var(--muted); font-weight: 600; text-transform: uppercase; letter-spacing: 0.08em; display: flex; align-items: center; gap: 0.3rem; }
    .td-code-display { font-family: var(--mono); font-size: 1.5rem; font-weight: 700; letter-spacing: 0.2em; color: var(--pink); background: rgba(232,121,249,0.08); border: 1.5px solid rgba(232,121,249,0.25); padding: 0.3rem 0.9rem; border-radius: var(--radius-sm); }
    .td-code-copy { display: flex; align-items: center; gap: 0.35rem; background: rgba(232,121,249,0.1); border: 1px solid rgba(232,121,249,0.25); color: var(--pink); padding: 0.3rem 0.8rem; border-radius: 6px; cursor: pointer; font-size: 0.78rem; font-weight: 600; font-family: var(--font); transition: background 0.2s, transform 0.1s; }
    .td-code-copy:hover { background: rgba(232,121,249,0.2); transform: translateY(-1px); }
    .td-code-hint { font-size: 0.68rem; color: #475569; }

    .td-tabs { display: flex; gap: 0.35rem; background: var(--bg2); border: 1px solid var(--border); padding: 0.35rem; border-radius: var(--radius); width: fit-content; margin-bottom: 1.5rem; }
    .td-tab { display: flex; align-items: center; gap: 0.4rem; padding: 0.5rem 1.1rem; border-radius: var(--radius-sm); border: none; background: transparent; color: var(--muted); font-size: 0.83rem; font-weight: 600; font-family: var(--font); cursor: pointer; transition: all 0.2s; }
    .td-tab:hover { color: var(--text); background: rgba(255,255,255,0.04); }
    .td-tab.active { background: var(--bg3); color: var(--accent); box-shadow: 0 1px 8px rgba(0,0,0,0.3); border: 1px solid var(--border); }
    .td-tab i { font-size: 0.8rem; }

    .td-panel { background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.25rem; transition: border-color 0.2s; }
    .td-panel:hover { border-color: rgba(255,255,255,0.11); }
    .td-panel-title { font-size: 0.85rem; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: 0.07em; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.45rem; }
    .td-panel-title i { color: var(--accent); }

    .td-stats-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 0.85rem; margin-bottom: 1.25rem; }
    .td-stat-card { background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.1rem 1rem; display: flex; flex-direction: column; gap: 0.25rem; position: relative; overflow: hidden; transition: transform 0.2s, border-color 0.2s; }
    .td-stat-card::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, var(--accent, #818cf8) 0%, transparent 60%); opacity: 0.04; pointer-events: none; }
    .td-stat-card:hover { transform: translateY(-2px); border-color: rgba(255,255,255,0.13); }
    .td-stat-icon { font-size: 1rem; margin-bottom: 0.3rem; }
    .td-stat-value { font-size: 1.6rem; font-weight: 800; color: var(--text); line-height: 1; }
    .td-stat-label { font-size: 0.72rem; font-weight: 600; color: var(--muted); text-transform: uppercase; letter-spacing: 0.06em; }
    .td-stat-sub { font-size: 0.68rem; color: #374151; }

    .td-two-col { display: grid; grid-template-columns: 1.4fr 1fr; gap: 1rem; }
    .td-col-right { display: flex; flex-direction: column; gap: 1rem; }
    .td-three-col { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; }

    .td-progress-row { display: flex; align-items: center; gap: 0.6rem; margin-bottom: 0.55rem; }
    .td-progress-label { font-size: 0.72rem; color: var(--muted); width: 150px; flex-shrink: 0; white-space: nowrap; }
    .td-progress-track { flex: 1; height: 6px; background: rgba(255,255,255,0.06); border-radius: 99px; overflow: hidden; }
    .td-progress-fill { height: 100%; border-radius: 99px; transition: width 0.6s cubic-bezier(.22,1,.36,1); }
    .td-progress-val { font-size: 0.72rem; color: var(--muted); font-family: var(--mono); width: 18px; text-align: right; }

    .td-spark-row { display: flex; align-items: flex-end; gap: 4px; height: 64px; padding: 0 0 4px; }
    .td-spark-bar { flex: 1; min-width: 6px; border-radius: 4px 4px 0 0; opacity: 0.85; transition: opacity 0.2s; cursor: default; }
    .td-spark-bar:hover { opacity: 1; }
    .td-spark-legend { font-size: 0.68rem; color: #374151; margin-top: 0.5rem; text-align: right; }
    .td-no-data { font-size: 0.8rem; color: var(--muted); display: flex; align-items: center; gap: 0.4rem; }

    .td-flag-row { display: flex; align-items: center; gap: 0.6rem; padding: 0.55rem 0; border-bottom: 1px solid var(--border); }
    .td-flag-row:last-child { border-bottom: none; }
    .td-flag-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
    ...
    .td-students-topbar { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem; }
    .td-filters { display: flex; gap: 0.4rem; flex-wrap: wrap; }
    .td-filter-btn { padding: 0.35rem 0.85rem; border-radius: 99px; border: 1px solid var(--border); background: transparent; color: var(--muted); font-size: 0.75rem; font-weight: 600; font-family: var(--font); cursor: pointer; transition: all 0.18s; }
    ...
    .td-search { padding: 0.4rem 0.9rem; border-radius: var(--radius-sm); border: 1px solid var(--border); background: var(--bg3); color: var(--text); font-size: 0.8rem; font-family: var(--font); outline: none; width: 200px; transition: border-color 0.2s; }
    .td-search:focus { border-color: var(--accent); }

    .td-roster-header { display: grid; grid-template-columns: 40px 1fr 72px 120px 80px 70px 90px 90px 24px; gap: 0.5rem; padding: 0 0.75rem 0.5rem; font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--muted); border-bottom: 1px solid var(--border); margin-bottom: 0.5rem; }
    .td-col { text-align: right; }

    .td-student-card { border: 1px solid var(--border); border-radius: var(--radius); margin-bottom: 0.5rem; overflow: hidden; transition: border-color 0.2s; }
    .td-student-card:hover { border-color: rgba(255,255,255,0.13); }
    .td-student-card.expanded { border-color: rgba(129,140,248,0.35); }
    .td-roster-row { display: grid; grid-template-columns: 40px 1fr 72px 120px 80px 70px 90px 90px 24px; gap: 0.5rem; align-items: center; padding: 0.7rem 0.75rem; cursor: pointer; background: var(--bg2); transition: background 0.15s; }
    .td-roster-row:hover { background: var(--bg3); }

    .td-avatar { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 0.72rem; font-weight: 800; flex-shrink: 0; }
    ...
    .td-chevron { color: var(--muted); font-size: 0.7rem; transition: transform 0.25s; justify-self: center; }
    .td-chevron.open { transform: rotate(180deg); }

    .td-student-detail { background: var(--bg3); border-top: 1px solid var(--border); padding: 1rem 1.25rem; animation: slideDown 0.2s ease; }
    @keyframes slideDown { from { opacity:0; transform:translateY(-8px); } to { opacity:1; transform:translateY(0); } }

    .td-flag-strip { display: flex; gap: 0.4rem; flex-wrap: wrap; margin-bottom: 0.85rem; }
    .td-flag-chip { font-size: 0.72rem; font-weight: 600; padding: 0.2rem 0.6rem; border-radius: 99px; background: rgba(251,191,36,0.1); border: 1px solid rgba(251,191,36,0.25); color: var(--amber); }
    .td-flag-chip.danger { background: rgba(248,113,113,0.1); border-color: rgba(248,113,113,0.25); color: var(--red); }

    .td-detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem; }
    .td-detail-panel { background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 0.9rem; }
    .td-detail-title { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--muted); margin-bottom: 0.75rem; }
    .td-detail-best { font-size: 0.75rem; color: var(--muted); margin-top: 0.5rem; }
    .td-detail-best strong { color: var(--text); }

    .td-stories-list { display: flex; flex-direction: column; gap: 0.5rem; }
    .td-story-row { display: flex; align-items: center; gap: 0.6rem; padding: 0.5rem 0; border-bottom: 1px solid var(--border); }
    ...
    .td-sbadge { font-size: 0.68rem; font-weight: 700; font-family: var(--mono); padding: 0.15rem 0.45rem; border-radius: 5px; }
    ...
    .td-expand-title { font-size: 0.78rem; font-weight: 700; color: var(--muted); margin: 1rem 0 0.5rem; }
    .td-milestone-row { display: flex; flex-wrap: wrap; gap: 0.5rem; }
    .td-milestone { display: flex; align-items: center; gap: 0.35rem; padding: 0.3rem 0.65rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; transition: all 0.2s; }
    .td-milestone.achieved { background: rgba(52,211,153,0.12); border: 1px solid rgba(52,211,153,0.3); color: var(--green); }
    .td-milestone.locked { background: rgba(75,85,99,0.12); border: 1px solid rgba(75,85,99,0.25); color: #475569; opacity: 0.7; }
    .td-milestone-icon { font-size: 0.9rem; }

    .td-milestone-student { border-bottom: 1px solid var(--border); padding: 1rem 0; }
    .td-milestone-student:last-child { border-bottom: none; }
    .td-milestone-student-header { display: flex; align-items: center; gap: 0.65rem; margin-bottom: 0.6rem; }

    .td-badge { font-size: 0.68rem; font-weight: 700; padding: 0.2rem 0.55rem; border-radius: 99px; display: inline-flex; align-items: center; gap: 0.3rem; white-space: nowrap; }
    ...
    .td-leader-row { display: flex; align-items: center; gap: 0.65rem; padding: 0.6rem 0; border-bottom: 1px solid var(--border); }
    .td-leader-row:last-child { border-bottom: none; }
    .td-leader-row > span:not(.td-rank):not(.td-badge) { flex: 1; font-size: 0.83rem; }

    .td-loading, .td-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1rem; min-height: 200px; color: var(--muted); }
    .td-spinner { width: 36px; height: 36px; border: 3px solid var(--border); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.7s linear infinite; }
    @keyframes spin { to { transform: rotate(360deg); } }

    /* Error banner */
    .td-error-banner { display: flex; align-items: center; gap: 0.75rem; padding: 1rem 1.25rem; background: rgba(248,113,113,0.08); border: 1px solid rgba(248,113,113,0.25); border-radius: var(--radius); margin-bottom: 1.25rem; font-size: 0.83rem; color: var(--red); }
    .td-error-banner i { flex-shrink: 0; }

    @media (max-width: 1100px) {
        .td-stats-grid { grid-template-columns: repeat(3, 1fr); }
        .td-two-col { grid-template-columns: 1fr; }
        .td-roster-header, .td-roster-row { grid-template-columns: 40px 1fr 72px 90px 72px 60px 80px 80px 24px; }
    }
    ...
    /* ─── Sub-components (unchanged from demo) ─── */
    ...
    return (
        ...
        {data.map((d, i) => (
            <div key={i} className="td-spark-bar"
    ...
    <div className="td-student-detail">
        ...
        <div className="td-detail-panel">
            ...
            <ProgressBar label="Slow (&lt;150 WPM)"  value={s.speedDist?.slow   ?? 0} max={s.totalStories} color="#f87171" />
            <ProgressBar label="Normal (150–250)"    value={s.speedDist?.normal ?? 0} max={s.totalStories} color="#34d399" />
            <ProgressBar label="Fast (&gt;250 WPM)"  value={s.speedDist?.fast   ?? 0} max={s.totalStories} color="#818cf8" />
            <div className="td-detail-best">Best: <strong>{s.bestWPM} WPM</strong></div>
        </div>
        <div className="td-detail-panel">
            <div className="td-detail-title">Comprehension</div>
            <ProgressBar label="Excellent (≥80%)"     value={s.compDist?.excellent  ?? 0} max={s.totalStories} color="#34d399" />
            <ProgressBar label="Good (60–79%)"         value={s.compDist?.good        ?? 0} max={s.totalStories} color="#fbbf24" />
            <ProgressBar label="Needs work (&lt;60%)" value={s.compDist?.needsWork   ?? 0} max={s.totalStories} color="#f87171" />
 ...
 useEffect(() => {
    const fetchDashboard = async () => {
        ...
        try {
            ...
            console.log('Dashboard response:', json); // ← add this
    ...
    if (!data) return (
        ...
        <div className="td-empty">
            ...
            <p>No data yet. Share your class code to enroll students.</p>
        </div>
        </>
    );

    const { summary, classTrend, attentionFlags, students, topPerformers, classCode } = data;
    ...
    return (
        ...
        <div className="td-wrap">
            ...
            <div>
                <h1 className="td-title"><i className="fa-solid fa-chart-line" /> Teacher Dashboard</h1>
            ...
            <div className="td-panel">
                ...
                <input className="td-search" type="text" placeholder="Search student…"
                    value={search}
                    onChange={(e) => { setSearch(e.target.value); setExpanded(null); }}
                />
                </div>
                <div className="td-roster-header">
                <span /><span>Student</span>
                ...
                : filteredStudents.map((s, i) => {
                    ...
                    const ac = avatarColors[s.status] ?? avatarColors.inactive;
                    return (
